...
$(function () {
  ...
  var testObj = {
    ...
    questions: [{
      quest: 'Вопрос № 1',
      answers: [{ answer: 'Вариант ответа № 1', check: true }, { answer: 'Вариант ответа № 2', check: false }, { answer: 'Вариант ответа № 3', check: false }]
    }, {
      quest: 'Вопрос № 2',
      answers: [{ answer: 'Вариант ответа № 1', check: false }, { answer: 'Вариант ответа № 2', check: true }, { answer: 'Вариант ответа № 3', check: false }]
    }, {
      quest: 'Вопрос № 3',
      answers: [{ answer: 'Вариант ответа № 1', check: false }, { answer: 'Вариант ответа № 2', check: false }, { answer: 'Вариант ответа № 3', check: true }]
    }]
  };
  ...
  function createTest() {
    ...
    function goodResult() {
      //функция отрабатывающая появление модального окна при правильном тесте
      $('.window_fon').css('display', 'block').click(function () {
        //по классу устанавливаем свойство стиля css и при клике запускаем функцию которая предшествует появлению модального окна
    ...
    $('#check_result').click(function () {
      //найденный элемент при клике запускает функцию проверки ответов
      ...
      if (checkedAnswers.length == correctAnswers.length) {
        //используем логический оператор для сравнения ответов на правильность
        var correctNum = 0; //переменная для правильных ответов

        for (var k = 0; k < checkedAnswers.length; k++) {
          //циклом проходим по ответам
          if (checkedAnswers[k].getAttribute('class') == 'correct') {
            //если(к ответу[к] применяем метод который вернет значение атрибута) и сделаем сравнение этого значения с классом на правильность
            correctNum++; //правильные ответы увеличиваем на 1
          }
        }
        if (correctNum == correctAnswers.length) {
          //сравниваем правильные ответы с переменной в которую записаны правельные ответы
