...
let testStr = JSON.stringify(testObj);//преобразовываем js обьект в JSON строку(JSON формат передачи данных от браузера к серверу и обратно)
localStorage.setItem('progrtest', testStr);//записываем на жесткий диск информацию об обьекте

function createTest(){ // функция теста
let test = localStorage.getItem('progrtest');//получаем обратно наше значение по ключу с хранилища
test = JSON.parse(test);//превращаем в js обьект (распарсили)

let html = $('#progrtest').html();//обьявили переменную и записали html в нее по id все совпавшие элементы
let content = tmpl(html, test);//обьявили переменную и присвоили ей метод tmpl c аргументами html(теги) и распарсенный js обьект тем самым запускаю шаблон
$('body').append(content);//в боди добавляем наш сшаблонизированный html
  ...
  function goodResult(){ //функция отрабатывающая появление модального окна при правильном тесте
   ...
   $('#check_result').click(function(){//найденный элемент при клике запускает функцию проверки ответов
    let checkedAnswers = $('#form-test input:checked');//обьявляем переменную и присваеваем ей элемент из $коллекции с указаным атрибутом
    let correctAnswers = $('.correct');//в переменную правильных ответов добавляем элемент с классом из $коллекции
    if (checkedAnswers.length == correctAnswers.length) {//используем логический оператор для сравнения ответов на правильность
      let correctNum = 0;//переменная для правильных ответов
